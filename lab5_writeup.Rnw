...
\begin{itemize}
\item The dataseet includes features such as \textbf{overall_loudness, tempo, danceability, and spectral_energy}
...
\section{Results}
\subssection{Key Findings}
\begin{itemize}
\item Overall_loudness, tempo, and energy, align more closely with Manchester Orchestra's data
